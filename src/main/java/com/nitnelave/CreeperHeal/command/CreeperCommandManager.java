...
public class CreeperCommandManager implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String commandLabel, String[] args) {
		if (args.length != 0) {
			...
			// in case of a change of setting via a command, write
			// it to the file
			CreeperConfig.write();
		} else {
			sender.sendMessage("/ch help");
	...
	private void sendHelp(CommandSender sender) {
		...
		if (!(admin || healNear))
			...
			if (healNear)
				sender.sendMessage(GREEN + "/ch healNear" + (healNearOther ? " (player)" : "") + " :" + PURPLE + " Heals all explosions around" + (healNearOther ? " the given player" : ""));
		}
	}
	 ...
	 *                The command's sender.
	 * @param currentWorld
	 *                The world to which the command must be applied. If
	 *                null, apply to all worlds.
	 */
	 ...
	 * 
	 * @param sender
	 *                The sender. If it is the console, then the command is
	 *                ignored.
	 * @param args
	 *                The command arguments.
	 */
