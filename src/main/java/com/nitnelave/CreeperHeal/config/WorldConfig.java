...
public class WorldConfig {
	...
	private BlockIdListValue replaceBlackList, griefPlaceList, protectList,
			replaceWhiteList;
	private final YamlConfiguration config = new YamlConfiguration(),
			advanced = new YamlConfiguration(),
			grief = new YamlConfiguration(),
			replacement = new YamlConfiguration();
	private final File worldFolder, configFile, advancedFile, griefFile,
			replaceFile;

	/**
	 * Main constructor. Load the config from the file, or create a default
	 * one if needed.
	 *
	 * @param name
	 *                The world's name.
	...
	/**
	 * Get whether damage caused by an entity should be replaced in this
	 * world.
	 *
	 * @param entity
	...
	public boolean isProtected(Block block) {
		return protectList.getValue().contains(new BlockId(block)) || (block.getState() instanceof InventoryHolder && CreeperConfig.getBool(CfgVal.REPLACE_PROTECTED_CHESTS) && PluginHandler.isProtected(block));
	}

	/**
	 * Get whether any grief protection feature is enabled, thus requiring
	 * to listen to the grief events.
	 *
	 * @return Whether any grief protection is enabled.
	 */
	public boolean hasGriefProtection() {
		return getBool(WCfgVal.BLOCK_LAVA) || getBool(WCfgVal.BLOCK_IGNITE) || getBool(WCfgVal.BLOCK_PVP) || getBool(WCfgVal.BLOCK_SPAWN_EGGS) || getBool(WCfgVal.BLOCK_TNT) || getBool(WCfgVal.WARN_IGNITE) || getBool(WCfgVal.WARN_LAVA) || getBool(WCfgVal.WARN_PVP) || getBool(WCfgVal.WARN_SPAWN_EGGS) || getBool(WCfgVal.WARN_TNT) || getBool(WCfgVal.PREVENT_FIRE_LAVA) || getBool(WCfgVal.PREVENT_FIRE_SPREAD) || (!griefPlaceList.getValue().isEmpty() && (getBool(WCfgVal.GRIEF_BLOCK_BLACKLIST) || getBool(WCfgVal.WARN_BLACKLIST)));
	}

	/**
	 * Get whether the block is in the grief blackList (or not in the
	 * whitelist, if the whitelist is used).
	 *
	 * @param block
	 *                The block to test.
	 ...
	 *                The value.
	 * @throws NullPointerException
	 *                 If the key is not a valid boolean configuration
	 *                 value.
	 */
	public void setBool(WCfgVal key, boolean value)
			throws NullPointerException {
	 ...
	 *                The value.
	 * @throws NullPointerException
	 *                 If the key is not a valid Integer configuration
	 *                 value.
	 */
	...
	/**
	 * Get whether a block is blacklisted for replacement (i.e. should not
	 * be replaced).
	 *
	 * @param id
