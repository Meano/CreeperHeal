...
public class DelayReplacement implements Runnable {
	 ...
	 *                The block to be replaced.
	 * @param replaced
	 *                The number of times a replacement has already been
	 *                attempted.
	 * @param reason
	 */
	 ...
	 * failure either re-schedule the replacement for later or drop the
	 * block.
	 */
	@Override
	public void run() {
		if (counter < REPLACEMENT_THRESHOLD) {
			if ((blockState instanceof Attachable && blockState.getBlock().getRelative(((Attachable) blockState).getAttachedFace()).getType() == Material.AIR) || blockState.getBlock().getRelative(BlockFace.DOWN).getType() == Material.AIR)
				counter++;
			else {
