...
public class ExplodedBlockManager {
	 ...
	 * Near is defined in the config by the parameter
	 * "advanced.distance-near".
	 * 
	 * @param target
	...
	/**
	 * Record all the blocks in the list, with the location as the source of
	 * the explosion.
	 * 
	...
	public static void processExplosion(List<Block> originalBlockList, Location location, CHExplosionRecordEvent.ExplosionReason reason) {
		...
		// process list is the list of blocks yet to be processed by
		// creeperheal.
		...
		/*
		 * Immediately replace the blocks marked for immediate
		 * replacement.
		 */
	 ...
	 */
	private static void checkReplace() { // check to see if any block has to
						// be replaced
		ListIterator<CreeperExplosion> iter = explosionList.listIterator();
		while (iter.hasNext()) {
			CreeperExplosion ex = iter.next();
	...
	/*
	 * Clean the explosion map from useless empty explosions. Do not use
	 * when in light weight mode.
	 */
	...
	/**
	 * Cancel and re-schedule the block replacement task, to update the
	 * block interval.
	 */
