...
public abstract class Suffocating {
	...
	public static void checkPlayerOneBlock(Location loc) {
		...
		for (Entity en : play_list)
			if (en instanceof LivingEntity && en.getWorld().equals(loc.getWorld()) && loc.distance(en.getLocation()) < 2)
				en.teleport(check_player_suffocate((LivingEntity) en));
	}

	/**
	 * Check the players and other animals (except in lightweight mode) to
	 * see if they were trapped by the explosion's replacement.
	 *
	 * @param loc
	...
	/*
	 * Check if the block at the coordinates, or one above or below is
	 * suitable to put a living being.
	 */
