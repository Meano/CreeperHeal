...
public class NeighborFire extends NeighborFinder<CreeperBurntBlock> {
	...
	public void clean() {
		...
		Date delay = new Date(new Date().getTime() - 1000 * CreeperConfig.getInt(CfgVal.WAIT_BEFORE_HEAL_BURNT) + 4000000 * CreeperConfig.getInt(CfgVal.BLOCK_PER_BLOCK_INTERVAL));
		while (iter.hasNext()) {
