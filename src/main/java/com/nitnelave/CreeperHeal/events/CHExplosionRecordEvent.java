...
public class CHExplosionRecordEvent extends Event implements Cancellable {
	...
	/**
	 * Blocks removed from this list will explode naturally if they were in
	 * the explosion.
	 * 
	 * Adding blocks to this list will mark them for deletion (if normally
	 * exploded (config dependent)), then healed when appropriate.
	 * 
	 * Recommended to use protectBlock, processBlock and explodeBlock for
	 * readability.
	 * 
	 * @return A mutable list of blocks that are currently listed for
	 *         CreeperHeal processing (not guaranteed to heal).
	 **/
	...
	/**
	 * Deprecated due to changed behavior. Now acts as it originally should
	 * have.
	 **/
	@Deprecated
	...
	/**
	 * Adds the block to the list for CreeperHeal to process (not guaranteed
	 * to heal depends on config).
	 */
	...
	/**
	 * Prevents CreeperHeal processing the block, letting
	 * minecraft/bukkit/plugins deal with the block as appropriate.
	 */
	...
	public enum ExplosionReason {
		CREEPER, TNT, DRAGON, OTHER, GHAST
	}
